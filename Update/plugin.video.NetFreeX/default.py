...
addon = xbmcaddon.Addon('plugin.video.NetFreeX')
...
Porn = addon.getSetting('Porn')	
Premium = addon.getSetting('Premium')				
Channel = base64.decodestring('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0tzcGVyc2t5L05ldEZyZWVYL21hc3Rlci9TZXJ2aWRvci9VcGRhdGVfQ2hhbm5lbA==')
Update = urllib2.urlopen(Channel).read()
Erotic = 'https://pastebin.com/raw/kpN9ivF4' 
Sexy = urllib2.urlopen(Erotic).read()
Order = addon.getSetting('Order')	
Filter = addon.getSetting('Filter')	
Server = base64.decodestring('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0tzcGVyc2t5L05ldEZyZWVYL21hc3Rlci9TZXJ2aWRvci9Ib3N0') 
Servidor = urllib2.urlopen(Server).read()
...
NFXbase = base64.decodestring('aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L1pteWJtZURY')

def NetFreeX():
    addon_log("NetFreeX")
    if Premium=='On':
        Login = 'http://'+Servidor+'/?key='+Order+'&filter='+Filter+'&user='+User+'&pass='+Pass+''
        Image = 'http://lista.redenext.com/background/1.jpg'  	
    if Premium=='Off':
        Login = 'http://'+Servidor+'/?key='+Order+'&filter='+Filter+'&user=free&pass=free'
        Image = 'http://lista.redenext.com/background/1.jpg'			
    if Porn=='On': addDir('[COLOR orange][B]Adulto +18[/B][/COLOR]','https://raw.githubusercontent.com/Kspersky/NetFreeX/master/Erotic/Option',1,'http://lista.redenext.com/background/6.png','http://lista.redenext.com/background/5.jpg','Atualização : '+Sexy,'','','','')	
    addDir('[COLOR yellow][B]Canais[/B][/COLOR]',Login,1,Image,'http://lista.redenext.com/background/0.jpg','Atualização : '+Update,'','','','')	
    getData(NFXbase,'')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.NetFreeX-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(NetFreeX,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(NetFreeX,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(NetFreeX,New source added.,5000,"+icon+")")
        if not url is None:
...
#                print channel
                ...
                try:
                    if linkedUrl=='':
                        addDir('[COLOR yellow][B]'+name.encode('utf-8', 'ignore')+'[/B][/COLOR]',url.encode('utf-8'),2,thumbnail,fanArt,desc,genre,date,credits,True)
                    else:
                        #print linkedUrl
                        addDir('[COLOR yellow][B]'+name.encode('utf-8')+'[/B][/COLOR]',linkedUrl.encode('utf-8'),1,thumbnail,fanArt,desc,genre,date,None,'source')
                except:
...
#                print 'proxy',proxy
            ...
            xbmc.executebuiltin("XBMC.Notification(NetFreeX,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(NetFreeX,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(NetFreeX,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(NetFreeX,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(NetFreeX,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
if not viewmode==None:
   ...
   xbmc.executebuiltin("Container.SetViewMode(%s)"%viewmode)
    ...
    addon_log("reload")    
    xbmcaddon.Addon('plugin.video.NetFreeX').openSettings()
    xbmc.executebuiltin("XBMC.Container.Refresh")